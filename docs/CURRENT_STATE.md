...
- A kernel and libc update was waiting for a reboot. Claude Code's permission check refused to reboot a production host, so the owner ran `systemctl reboot` themselves. The host came back on `7.0.0-31` with Caddy, Docker, the orchestrator, the worker and the workspace container all running, `/var/run/reboot-required` cleared, and no warnings in the logs.
- Hardened at the owner's request: SSH is key-only (a drop-in sets `PasswordAuthentication no`; a client without a key now gets `Permission denied (publickey)`), and `ufw` is active, allowing 22/tcp, 80/tcp, 443/tcp and 443/udp. Each change was applied with a `systemd-run` timer that would have undone it after 5 minutes, and the timer was cancelled only after a fresh key login succeeded (and, for `ufw`, HTTPS, the redirect, host-to-agent traffic, container egress to GitHub and the Anthropic API, DNS and the reaper's passes were all confirmed, with no internal traffic blocked).
- Rebooted again at the owner's request to prove the hardening survives: `ufw` came back active with all its rules, Docker's `FORWARD` jumps still ahead of ufw's, sshd still key-only, every service running, the workspace container healthy and its networking intact, no warnings logged and no internal traffic blocked.
- Production accounts, checked and set at the owner's request: `andrej@notea.mk` already existed and owns the `notea` workspace; `niche@notea.mk` did not, and was created (agent uid 20003). Both passwords were set with `create-user` on the host and checked against the stored hashes with the app's own `verifyPassword`.
