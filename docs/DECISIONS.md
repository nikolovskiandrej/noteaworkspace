...
**Cost.** A run is no longer a PTY session a human can attach to live; the run event log is what the UI shows. Live attachment would mean teaching the daemon to adopt a process it did not spawn, which is not worth reopening the exposure for. Cancellation compensates by signalling the process group (`setsid` + a recorded pid), so a CLI's child tools die with the run.
