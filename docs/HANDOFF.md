...
3. **A credential for Niche** (he has a production account, `niche@notea.mk`, since session 11, and is an editor in `notea`), so the second member can run agents on his own account. The mechanism is built and proven for one member; it has never been exercised with two credential owners at once.
